...
class QKVNormRoPE(Operation):
    """Provider-specific Q/K/V normalization followed by RoPE."""
    ...
    def load_data(cls, database: PerfDatabase) -> None:
        ...
        if key not in cls._data_cache:
            ...
            primary_path = os.path.join(
                data_dir,
                PerfDataFilename.step4_qkv_norm_rope.value,
            )
            sources = database._build_op_sources(
                PerfDataFilename.step4_qkv_norm_rope,
                primary_path,
                system_data_root,
            )
            cls._data_cache[key] = LoadedOpData(
                load_step4_qkv_norm_rope_data(sources),
                PerfDataFilename.step4_qkv_norm_rope,
                primary_path,
            )
            cls._record_load()

        if "_step4_qkv_norm_rope_data" not in database.__dict__:
            database._step4_qkv_norm_rope_data = cls._data_cache[key]

    def query(self, database: PerfDatabase, **kwargs) -> PerformanceResult:
        """Query an exact provider/structure and interpolate only token count."""
        from aiconfigurator.sdk import interpolation

        num_tokens = kwargs.get("x")
        if not isinstance(num_tokens, int) or isinstance(num_tokens, bool) or num_tokens <= 0:
            raise ValueError("QKVNormRoPE requires a positive num_tokens integer.")
        num_tokens = -(-num_tokens // self._seq_split)

        self.load_data(database)
        data = database._step4_qkv_norm_rope_data
        data.raise_if_not_loaded()
        structural_key = self._persisted_key()
        if structural_key not in data:
            raise PerfDataNotAvailableError(
                "QKV norm/RoPE perf data not available for exact provider/structure key. "
                f"system='{database.system}', backend='{database.backend}', "
                f"version='{database.version}', provider='{self._provider}', "
                f"normalized_tensors={self._normalized_tensors!r}, "
                f"q_heads={self._q_heads}, kv_heads={self._kv_heads}, "
        ...
        if num_tokens in token_data:
            ...
            try:
                ...
                raise PerfDataNotAvailableError(
                    "QKV norm/RoPE perf data does not bracket the requested token count. "
                    f"provider='{self._provider}', "
                    f"normalized_tensors={self._normalized_tensors!r}, "
                    f"q_heads={self._q_heads}, kv_heads={self._kv_heads}, "
                    f"head_dim={self._head_dim}, num_tokens={num_tokens}, "
                    f"available_tokens={token_points}."
                ) from exc
...
def load_step4_qkv_norm_rope_data(perf_file):
    """Load pinned Step4 QKV norm/RoPE rows keyed by structure then tokens."""
    rows = _read_filtered_rows(perf_file)
    if rows is None:
        return None

    qkv_data: dict[tuple, dict[int, dict[str, float]]] = {}
    for row in rows:
        provider = str(row["provider"])
        kernel_source = str(row["kernel_source"])
        op_name = str(row["op_name"])
        if op_name != "step4_qkv_norm_rope":
            raise ValueError(f"unexpected op_name in QKV norm/RoPE data: {op_name!r}")
        if kernel_source != provider:
            raise ValueError(
                "QKV norm/RoPE provider does not match kernel_source: "
                f"provider={provider!r}, kernel_source={kernel_source!r}"
            )

        normalized_tensors = tuple(part for part in str(row["normalized_tensors"]).split("+") if part)
        if not normalized_tensors:
            raise ValueError("QKV norm/RoPE row has no normalized tensors")
        structural_key = (
            provider,
            normalized_tensors,
            int(row["q_heads"]),
        ...
        token_data = qkv_data.setdefault(structural_key, {})
        existing = token_data.get(num_tokens)
        if existing is not None and existing != value:
            raise ValueError(
                "conflicting QKV norm/RoPE row for physical key "
                f"{(*structural_key, num_tokens)!r}: {existing!r} != {value!r}"
            )
        token_data[num_tokens] = value
