"""Run the fresh disaggregated Step4-Pro-V1/DeepSeek throughput-ratio matrix."""
...
MODELS = ("stepfun-ai/Step4-Pro-V1", "deepseek-ai/DeepSeek-V4-Pro")
...
THROUGHPUT_RUNNER_RELATIVE_PATH = "tests/performance/aic_roofline_pareto/run_dsv4pro_vs_step4pro_throughput.py"
...
DELTA_CONTRACT = {
    "absolute": "Step4-Pro-V1 - DeepSeek-V4-Pro",
    "relative": "(Step4-Pro-V1 - DeepSeek-V4-Pro) / DeepSeek-V4-Pro",
...
def build_model_comparisons(
    ...
    *,
    ...
    """Compare aligned Step4-Pro-V1 rank-one rows against DeepSeek-V4-Pro."""
    if not metrics:
    ...
    for aligned_key in sorted(aligned_keys, key=repr):
        ...
        for metric in metrics:
            candidate_value = base._finite_number(candidate[metric], field=f"Step4-Pro-V1 {metric}")
...
def _render_markdown_report(artifact: Mapping[str, Any]) -> str:
    """Render the reference report schema with explicit Step4-Pro-V1 semantics."""
    summary = artifact["summary"]
    lines = [
        "# Step4-Pro-V1 vs DeepSeek-V4-Pro SOL Comparison",
        "",
        "## Summary",
        "",
        "| Metric | Value |",
        ...
        "## Modeling Boundary",
        "",
        "Step4-Pro-V1 uses 20 explicit standard full-attention layers and 60 explicit ratio-128 HCA layers. "
        "No legacy Step4 MLA substitute or SWA-to-HCA fallback is used in DatabaseMode.SOL.",
        "",
        "Full-attention runtime KV representation, HCA checkpoint tensor fidelity, FP8 tensor details, and "
    ...
    lines.extend(
        [
            ...
            "## Paired Model Deltas",
            "",
            "Absolute delta is Step4-Pro-V1 minus DeepSeek-V4-Pro. "
            "TPOT is observational and does not affect eligibility.",
            "",
            "| Metric | Step4-Pro-V1 | DeepSeek-V4-Pro | Absolute delta | Relative delta | Polarity | Status |",
            "|---|---:|---:|---:|---:|---|---|",
        ]
    )
