"""Unit coverage for the fresh Step4/DeepSeek throughput-ratio workflow."""
...
from tests.performance.aic_roofline_pareto import plot_throughput_ratio as plotting
from tests.performance.aic_roofline_pareto import run_dsv4pro_vs_step4_throughput as runner

pytestmark = pytest.mark.unit

BASE_RUNNER_SOURCE = "tests/performance/aic_roofline_pareto/run_step4_comparison.py"
THROUGHPUT_RUNNER_SOURCE = "tests/performance/aic_roofline_pareto/run_dsv4pro_vs_step4_throughput.py"
...
def test_matrix_uses_only_disagg_and_uniform_osl_and_sla():
    ...
    step4_points = [point for point in points if point.model == "stepfun-ai/Step4"]
    assert all(point.attention_approximation == "temporary_mla_substitute" for point in step4_points)
    assert {point.isl for point in step4_points if point.approximation_dominated} == {65_536, 131_072}
...
def test_ratio_direction_is_step4_over_deepseek():
    rows = [
        {
            "rank": 1,
            ...
            "system": "h200_sxm",
            "isl": 1024,
            "prefill_input_throughput": 120.0,
        ...
        },
        {
            ...
            "system": "h200_sxm",
            "isl": 1024,
            "prefill_input_throughput": 100.0,
            ...
            "ranked_rows": [
                {
                    ...
                    "isl": isl,
                    "serving_mode": "disagg",
                    "prefill_input_throughput": 100.0 if model.startswith("deepseek") else 120.0,
                    "output_token_throughput": 50.0 if model.startswith("deepseek") else 75.0,
                }
                for model in ("stepfun-ai/Step4", "deepseek-ai/DeepSeek-V4-Pro")
