...
def test_step4_pro_v1_rejects_non_formula_database_modes(serving_mode, database_mode):
    """The Pro identity must inherit the strict Step4 formula-only Task guard."""
...
def test_step4_pro_v1_formula_validation_never_loads_perfdb(monkeypatch, serving_mode, database_mode):
    """Formula-only Task validation must finish before any database load."""
    task = Task(**_task_kwargs(serving_mode, database_mode))
...
def _direct_formula_queries(database):
    ...
    return {
        ...
        "context_mla": lambda mode: database.query_context_mla(
            2,
            4096,
            0,
            16,
        ...
        "generation_mla": lambda mode: database.query_generation_mla(
            2,
            4096,
            16,
            common.KVCacheQuantMode.fp8,
        ...
        "mla_bmm": lambda mode: database.query_mla_bmm(
            2,
            16,
            common.GEMMQuantMode.fp8,
            True,
...
def test_step4_pro_v1_sol_full_graph_fails_at_the_shared_tuple_contract():
    """Document that direct tuple audit is not end-to-end SOL_FULL support."""
    task = Task(
...
def test_step4_pro_v1_decode_consumes_all_generation_attention_components():
    """Decode must retain every Full/SWA projection, BMM, MLA, and AR result."""
    task = Task(
        **_task_kwargs("agg", "SOL"),
        isl=128,
        osl=2,
